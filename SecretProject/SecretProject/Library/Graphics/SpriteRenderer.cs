...
namespace SecretProject.Library.Objects
{
	public struct SpriteRenderer
	{
		...
		public Vector2 renderOrigin => new Vector2 (0, sprite.height);
		...
		public void Draw(GameTime gameTime, SpriteBatch spriteBatch, Vector2 drawPos, SpriteEffects sprFx, int skipFrame = -1)
		{
			if (sprite is null) return;
						...
						if (sprite.animIsLooping)
						{
							if (skipFrame != -1)
							{
								...
								while (!spriteValid)
								{
									...
									if (sprite.animFrameIndex != skipFrame) spriteValid = true;
								}
							}
							else
							{
